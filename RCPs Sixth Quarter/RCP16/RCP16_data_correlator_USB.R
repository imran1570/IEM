#==============================================================================
#title           :	RCP16_data_correlator_USB.R
...
#copula_type = "t" #default

#dist_sample_type = "triangular" #default
...
#cop_sample_df$staff_id <- sampled_all_dist_df$staff_id
...
tri_t_correlated_data_df_list <- list()
 ...
 for (sid in staff_ids){
   ...
   gauss_user_df <- subset(filter(gaussian_sampled_all_dist_df, staff_id == sid), select=-c(staff_id, group_id, day))
   
   tri_user_df <- subset(filter(triangular_sampled_all_dist_df, staff_id == sid), select=-c(staff_id, group_id, day))
   
   # user_corr_df <- cor(user_df, method = "spearman") #useless just for the sake of input argument to function for dimension purposes
   # 
   # user_corr_df[is.na(user_corr_df)] <- 0 #useless just for the sake of input argument to function for dimension purposes
   
   norm_user_cop_sample_df <- subset(filter(norm_cop_sample_df, staff_id == sid), select = -c(staff_id, group_id))
   
   t_user_cop_sample_df <- subset(filter(t_cop_sample_df, staff_id == sid), select = -c(staff_id, group_id))
...
colnames(gauss_norm_correlated_data_df) <- colnames(gaussian_sampled_all_dist_df[-1]) #remove group_id from column name
...
colnames(gauss_t_correlated_data_df) <- colnames(gaussian_sampled_all_dist_df[-1]) #remove group_id from column name
...
colnames(tri_norm_correlated_data_df) <- colnames(gaussian_sampled_all_dist_df[-1]) #remove group_id from column name
...
colnames(tri_t_correlated_data_df) <- colnames(gaussian_sampled_all_dist_df[-1]) #remove group_id from column name
...
# correlated_data_df = correlated_data_df[colnames(subset(features, select=-c(Search_Threat, Exfil_Threat, Windows_User)))]
# 
# future_data_df <- correlated_data_df
# 
# future_data_df$staff_id = c(features$staff_id, features$staff_id)
# 
# future_data_df$day = c(features$day+10, features$day+20)
# 
# future_data_df = future_data_df[colnames(subset(features, select=-c(Search_Threat, Exfil_Threat, Windows_User)))]
# 
# 
# full_data_df = as.data.frame(rbind(subset(features, select=-c(Search_Threat, Exfil_Threat, Windows_User)), future_data_df))
