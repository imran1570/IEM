#==============================================================================
#title           :	RCP16_classifier_USB.R
#description     :	This code uses the dataset which has USB activity for non-windows user. Adds class labels to the data and then applies Naive Bayes and Random Forest classifiers to the data
...
for (num_org in start_org:end_org){
  ...
  gauss_norm_full_data_df <- as.data.frame(rbind(subset(features, select=-c(Search_Threat, Exfil_Threat, Windows_User, group_id)), filter(gauss_norm_six_week_data_df, day %in% estimated_days)))
  gauss_norm_full_data_df <- gauss_norm_full_data_df[order(gauss_norm_full_data_df[,c("staff_id")], gauss_norm_full_data_df[,c("day")]),]
  
  gauss_t_full_data_df <- as.data.frame(rbind(subset(features, select=-c(Search_Threat, Exfil_Threat, Windows_User, group_id)), filter(gauss_t_six_week_data_df, day %in% estimated_days)))
  gauss_t_full_data_df <- gauss_t_full_data_df[order(gauss_t_full_data_df[,c("staff_id")], gauss_t_full_data_df[,c("day")]),]
  
  tri_norm_full_data_df <- as.data.frame(rbind(subset(features, select=-c(Search_Threat, Exfil_Threat, Windows_User, group_id)), filter(tri_norm_six_week_data_df, day %in% estimated_days)))
  tri_norm_full_data_df <- tri_norm_full_data_df[order(tri_norm_full_data_df[,c("staff_id")], tri_norm_full_data_df[,c("day")]),]
  
  tri_t_full_data_df <- as.data.frame(rbind(subset(features, select=-c(Search_Threat, Exfil_Threat, Windows_User, group_id)), filter(tri_t_six_week_data_df, day %in% estimated_days)))
  ...
  if (file.exists(gauss_norm_six_week_data) &
    ...
    gauss_norm_full_data_df2 <- as.data.frame(rbind(subset(features, select=-c(Search_Threat, Exfil_Threat, Windows_User, group_id)), filter(gauss_norm_six_week_data_df2, day %in% estimated_days)))
    ...
    gauss_t_full_data_df2 <- as.data.frame(rbind(subset(features, select=-c(Search_Threat, Exfil_Threat, Windows_User, group_id)), filter(gauss_t_six_week_data_df2, day %in% estimated_days)))
    gauss_t_full_data_df2 <- gauss_t_full_data_df[order(gauss_t_full_data_df2[,c("staff_id")], gauss_t_full_data_df2[,c("day")]),]
    
    tri_norm_full_data_df2 <- as.data.frame(rbind(subset(features, select=-c(Search_Threat, Exfil_Threat, Windows_User, group_id)), filter(tri_norm_six_week_data_df2, day %in% estimated_days)))
    tri_norm_full_data_df2 <- tri_norm_full_data_df2[order(tri_norm_full_data_df2[,c("staff_id")], tri_norm_full_data_df2[,c("day")]),]
    
    tri_t_full_data_df2 <- as.data.frame(rbind(subset(features, select=-c(Search_Threat, Exfil_Threat, Windows_User, group_id)), filter(tri_t_six_week_data_df2, day %in% estimated_days)))
    ...
    gauss_norm_given_data_with_prop_feat_df2$Search_Threat <- as.factor(features$Search_Threat)
    gauss_norm_given_data_with_prop_feat_df2$Exfil_Threat <- as.factor(features$Exfil_Threat)
    ...
    gauss_t_given_data_with_prop_feat_df2$Search_Threat <- as.factor(features$Search_Threat)
    gauss_t_given_data_with_prop_feat_df2$Exfil_Threat <- as.factor(features$Exfil_Threat)
    ...
    tri_norm_given_data_with_prop_feat_df2$Search_Threat <- as.factor(features$Search_Threat)
    tri_norm_given_data_with_prop_feat_df2$Exfil_Threat <- as.factor(features$Exfil_Threat)
    ...
    tri_t_given_data_with_prop_feat_df2$Search_Threat <- as.factor(features$Search_Threat)
    tri_t_given_data_with_prop_feat_df2$Exfil_Threat <- as.factor(features$Exfil_Threat)
...
answers_df <- as.data.frame(cbind(nb_exfil_precisions, nb_exfil_recalls, nb_exfil_fprs,
                                  rf_exfil_precisions, rf_exfil_recalls, rf_exfil_fprs))

write_csv(answers_df, "./answers_13-18.csv")
