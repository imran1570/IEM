...
nb_search_precisions <- c()

nb_exfil_precisions <- c()
...
########### code block for naive bayes search threat classifier ################
  
  
  levels(gauss_norm_given_data_with_prop_feat_df$Search_Threat) <- make.names(levels((gauss_norm_given_data_with_prop_feat_df$Search_Threat))) #required for making models
  levels(gauss_norm_given_data_with_prop_feat_df$Exfil_Threat) <- make.names(levels((gauss_norm_given_data_with_prop_feat_df$Exfil_Threat))) #required for making models
  
  levels(gauss_t_given_data_with_prop_feat_df$Search_Threat) <- make.names(levels((gauss_t_given_data_with_prop_feat_df$Search_Threat))) #required for making models
  levels(gauss_t_given_data_with_prop_feat_df$Exfil_Threat) <- make.names(levels((gauss_t_given_data_with_prop_feat_df$Exfil_Threat))) #required for making models
  
  levels(tri_norm_given_data_with_prop_feat_df$Search_Threat) <- make.names(levels((tri_norm_given_data_with_prop_feat_df$Search_Threat))) #required for making models
  levels(tri_norm_given_data_with_prop_feat_df$Exfil_Threat) <- make.names(levels((tri_norm_given_data_with_prop_feat_df$Exfil_Threat))) #required for making models
  
  levels(tri_t_given_data_with_prop_feat_df$Search_Threat) <- make.names(levels((tri_t_given_data_with_prop_feat_df$Search_Threat))) #required for making models
  levels(tri_t_given_data_with_prop_feat_df$Exfil_Threat) <- make.names(levels((tri_t_given_data_with_prop_feat_df$Exfil_Threat))) #required for making models
  ...
  # train the model
  gauss_norm_nb_search_model <- train(Search_Threat ~ .,
                           data=subset(gauss_norm_given_data_with_prop_feat_df, select = -c(staff_id, day, Exfil_Threat)), trControl=train_control, method="nb", tuneGrid =grid,  metric="ROC")
  
  gauss_t_nb_search_model <- train(Search_Threat ~ .,
                                      data=subset(gauss_t_given_data_with_prop_feat_df, select = -c(staff_id, day, Exfil_Threat)), trControl=train_control, method="nb", tuneGrid =grid,  metric="ROC")
  
  tri_norm_nb_search_model <- train(Search_Threat ~ .,
                                      data=subset(tri_norm_given_data_with_prop_feat_df, select = -c(staff_id, day, Exfil_Threat)), trControl=train_control, method="nb", tuneGrid =grid,  metric="ROC")
  
  tri_t_nb_search_model <- train(Search_Threat ~ .,
                                      data=subset(tri_t_given_data_with_prop_feat_df, select = -c(staff_id, day, Exfil_Threat)), trControl=train_control, method="nb", tuneGrid =grid,  metric="ROC")
  

  # gauss norm model against 4 configurations
  predictions <- predict(gauss_norm_nb_search_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
  cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
  ...
  nb_search_precisions <- c(nb_search_precisions, search_precision)
  nb_search_recalls <- c(nb_search_recalls, search_recall)
  nb_search_fprs <- c(nb_search_fprs, search_fpr)
  
  predictions <- predict(gauss_norm_nb_search_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
  cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
  search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
  search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
  search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
  nb_search_precisions <- c(nb_search_precisions, search_precision)
  ...
  predictions <- predict(gauss_norm_nb_search_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
  cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
  search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
  search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
  search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
  nb_search_precisions <- c(nb_search_precisions, search_precision)
  ...
  predictions <- predict(gauss_norm_nb_search_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
  cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
  search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
  search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
  search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
  nb_search_precisions <- c(nb_search_precisions, search_precision)
...
#   predictions <- predict(gauss_t_nb_search_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
#   search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_search_fprs <- c(nb_search_fprs, search_fpr)
#   
#   predictions <- predict(gauss_t_nb_search_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
#   search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_search_fprs <- c(nb_search_fprs, search_fpr)
#   
#   predictions <- predict(gauss_t_nb_search_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
#   search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_search_fprs <- c(nb_search_fprs, search_fpr)
#   
#   predictions <- predict(gauss_t_nb_search_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
#   search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   predictions <- predict(tri_norm_nb_search_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
#   search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_search_fprs <- c(nb_search_fprs, search_fpr)
#   
#   predictions <- predict(tri_norm_nb_search_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
#   search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_search_fprs <- c(nb_search_fprs, search_fpr)
#   
#   predictions <- predict(tri_norm_nb_search_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
#   search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_search_fprs <- c(nb_search_fprs, search_fpr)
#   
#   predictions <- predict(tri_norm_nb_search_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
#   search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   predictions <- predict(tri_t_nb_search_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
#   search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_search_fprs <- c(nb_search_fprs, search_fpr)
#   
#   predictions <- predict(tri_t_nb_search_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
#   search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_search_fprs <- c(nb_search_fprs, search_fpr)
#   
#   predictions <- predict(tri_t_nb_search_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
#   search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_search_fprs <- c(nb_search_fprs, search_fpr)
#   
#   predictions <- predict(tri_t_nb_search_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
#   search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
############### code block for naive bayes exfil threat classifier ###############
  
  grid <- expand.grid(.fL = c(0), .usekernel=c(FALSE), .adjust=c(1))
  # define training control
  train_control <- trainControl(method="cv", number=10, summaryFunction = twoClassSummary, classProbs = TRUE)
  # train the model
  gauss_norm_nb_exfil_model <- train(Exfil_Threat ~ .,
                                      data=subset(gauss_norm_given_data_with_prop_feat_df, select = -c(staff_id, day, Search_Threat)), trControl=train_control, method="nb", tuneGrid =grid,  metric="ROC")
  
  gauss_t_nb_exfil_model <- train(Exfil_Threat ~ .,
                                   data=subset(gauss_t_given_data_with_prop_feat_df, select = -c(staff_id, day, Search_Threat)), trControl=train_control, method="nb", tuneGrid =grid,  metric="ROC")
  
  tri_norm_nb_exfil_model <- train(Exfil_Threat ~ .,
                                    data=subset(tri_norm_given_data_with_prop_feat_df, select = -c(staff_id, day, Search_Threat)), trControl=train_control, method="nb", tuneGrid =grid,  metric="ROC")
  
  tri_t_nb_exfil_model <- train(Exfil_Threat ~ .,
                                 data=subset(tri_t_given_data_with_prop_feat_df, select = -c(staff_id, day, Search_Threat)), trControl=train_control, method="nb", tuneGrid =grid,  metric="ROC")
  
  
  # gauss norm model against 4 configurations
  predictions <- predict(gauss_norm_nb_exfil_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
  cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
  ...
  nb_exfil_precisions <- c(nb_exfil_precisions, exfil_precision)
  nb_exfil_recalls <- c(nb_exfil_recalls, exfil_recall)
  nb_exfil_fprs <- c(nb_exfil_fprs, exfil_fpr)
  
  predictions <- predict(gauss_norm_nb_exfil_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
  cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
  exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
  exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
  exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
  nb_exfil_precisions <- c(nb_exfil_precisions, exfil_precision)
  ...
  predictions <- predict(gauss_norm_nb_exfil_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
  cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
  exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
  exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
  exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
  nb_exfil_precisions <- c(nb_exfil_precisions, exfil_precision)
  ...
  predictions <- predict(gauss_norm_nb_exfil_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
  cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
  exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
  exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
  exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
  nb_exfil_precisions <- c(nb_exfil_precisions, exfil_precision)
...
#   # gauss t model against 4 configurations
#   predictions <- predict(gauss_t_nb_exfil_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
#   exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_exfil_fprs <- c(nb_exfil_fprs, exfil_fpr)
#   
#   predictions <- predict(gauss_t_nb_exfil_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
#   exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_exfil_fprs <- c(nb_exfil_fprs, exfil_fpr)
#   
#   predictions <- predict(gauss_t_nb_exfil_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
#   exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_exfil_fprs <- c(nb_exfil_fprs, exfil_fpr)
#   
#   predictions <- predict(gauss_t_nb_exfil_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
#   exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   
#   # tri norm model against 4 configurations
#   predictions <- predict(tri_norm_nb_exfil_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
#   exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_exfil_fprs <- c(nb_exfil_fprs, exfil_fpr)
#   
#   predictions <- predict(tri_norm_nb_exfil_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
#   exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_exfil_fprs <- c(nb_exfil_fprs, exfil_fpr)
#   
#   predictions <- predict(tri_norm_nb_exfil_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
#   exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_exfil_fprs <- c(nb_exfil_fprs, exfil_fpr)
#   
#   predictions <- predict(tri_norm_nb_exfil_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
#   exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   
#   # tri t model against 4 configurations
#   predictions <- predict(tri_t_nb_exfil_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
#   exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_exfil_fprs <- c(nb_exfil_fprs, exfil_fpr)
#   
#   predictions <- predict(tri_t_nb_exfil_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
#   exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_exfil_fprs <- c(nb_exfil_fprs, exfil_fpr)
#   
#   predictions <- predict(tri_t_nb_exfil_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
#   exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
#   nb_exfil_fprs <- c(nb_exfil_fprs, exfil_fpr)
#   
#   predictions <- predict(tri_t_nb_exfil_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df, select = -c(staff_id, day)))
#   cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
#   exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
#   exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
#   exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
# ####################################################################################  
# 
#   print(as.data.frame(cbind(nb_search_precisions, nb_search_recalls, nb_search_fprs, nb_exfil_precisions, nb_exfil_recalls, nb_exfil_fprs)))
  ...
  if (file.exists(gauss_norm_six_week_data) &
    ...
    # gauss norm model against 4 configurations
    predictions <- predict(gauss_norm_nb_search_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(gauss_norm_nb_search_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(gauss_norm_nb_search_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(gauss_norm_nb_search_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    # gauss t model against 4 configurations
    predictions <- predict(gauss_t_nb_search_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(gauss_t_nb_search_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(gauss_t_nb_search_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(gauss_t_nb_search_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    # tri norm model against 4 configurations
    predictions <- predict(tri_norm_nb_search_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(tri_norm_nb_search_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(tri_norm_nb_search_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(tri_norm_nb_search_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    # tri t model against 4 configurations
    predictions <- predict(tri_t_nb_search_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(tri_t_nb_search_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(tri_t_nb_search_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(tri_t_nb_search_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Search_Threat, mode = "prec_recall", positive = "X1")
    search_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    search_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    search_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    # gauss norm model against 4 configurations
    predictions <- predict(gauss_norm_nb_exfil_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(gauss_norm_nb_exfil_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(gauss_norm_nb_exfil_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(gauss_norm_nb_exfil_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    # gauss t model against 4 configurations
    predictions <- predict(gauss_t_nb_exfil_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(gauss_t_nb_exfil_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(gauss_t_nb_exfil_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(gauss_t_nb_exfil_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    # tri norm model against 4 configurations
    predictions <- predict(tri_norm_nb_exfil_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(tri_norm_nb_exfil_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(tri_norm_nb_exfil_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(tri_norm_nb_exfil_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    # tri t model against 4 configurations
    predictions <- predict(tri_t_nb_exfil_model, newdata = subset(gauss_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(tri_t_nb_exfil_model, newdata = subset(gauss_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = gauss_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(tri_t_nb_exfil_model, newdata = subset(tri_norm_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_norm_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
    ...
    predictions <- predict(tri_t_nb_exfil_model, newdata = subset(tri_t_estimated_data_with_prop_feat_df2, select = -c(staff_id, day)))
    cfm_obj <- confusionMatrix(data = predictions, reference = tri_t_given_data_with_prop_feat_df$Exfil_Threat, mode = "prec_recall", positive = "X1")
    exfil_precision <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[2,1])
    exfil_recall <- cfm_obj$table[2,2] / (cfm_obj$table[2,2] + cfm_obj$table[1,2])
    exfil_fpr <- cfm_obj$table[2,1] / (cfm_obj$table[2,1] + cfm_obj$table[1,1])
...
answers_df <- as.data.frame(cbind(nb_search_precisions, nb_search_recalls, nb_search_fprs, nb_exfil_precisions, nb_exfil_recalls, nb_exfil_fprs))

#write_csv(answers_df, str_c(output_directory_name,"/answers_1-12.csv"))


write_csv(answers_df, "./answers_1-6.csv")



#View(cor(as.matrix(subset(gauss_norm_given_data_with_prop_feat_df, select=-c(staff_id, day, Search_Threat, Exfil_Threat)))) - cor(as.matrix(subset(gauss_norm_estimated_data_with_prop_feat_df, select=-c(staff_id, day)))))



# answers_without_usb.csv


# Search_Threat ~ Email_Sent+Avg_Attach_Sent+Total_Attach_Sent+
#   Avg_Ext_Attach_Sent+Total_Ext_Attach_Sent+Percent_with_attach+
#   Percent_Ext_Sent+Email_Recv+Avg_Attach_Recv+Total_Attach_Recv+
#   Avg_Ext_Attach_Recv+Total_Ext_Attach_Recv+Percent_Attach_Recv+
#   Percent_Ext_Attach_Rec+USB_Transfers+Avg_USB_Transfer+Total_USB_Transfer+
#   Searches+Websites+Avg_Website_size+Total_Website_Size+
#   Avg_Website_Ext+Total_Website_Ext+Percent_Ext_Website+Accesses+Unique_Accesses
