...
assumption_one_months <- c("2015-06", "2016-01", "2016-02") #july august september are similar to june january february
...
list_of_all_detector_samples_leave_train <- lapply(list_of_all_detector_samples_leave_train, function(df){df[sample(nrow(df)),]})
list_of_all_detector_samples_noleave_train <- lapply(list_of_all_detector_samples_noleave_train, function(df){df[sample(nrow(df)),]})
list_of_all_detector_samples_leave_test <- lapply(list_of_all_detector_samples_leave_test, function(df){df[sample(nrow(df)),]})
list_of_all_detector_samples_noleave_test <- lapply(list_of_all_detector_samples_noleave_test, function(df){df[sample(nrow(df)),]})
...
population_files <- list.files(str_c(getwd(), "/populations/"))
...
getwd()
write_csv(up_answers_df, "up_answers.csv")
write_csv(smote_answers_df, "smote_answers.csv")
