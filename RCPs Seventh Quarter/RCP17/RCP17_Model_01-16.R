...
#### code block to correlate training data across time / temporal correlations ####
...
### transforming the data into detector distribution by month in each matrix. Each matrix contains the value of a particular detector for all months
...
for (det in all_detectors){
  counter <- counter+1
  for (month in given_months){
    dist_list[[month]] <- list_of_all_detector_samples_leave_train[[month]][,counter]
  }
...
for (det in all_detectors){
  counter <- counter+1
  for (month in given_months){
    dist_list[[month]] <- list_of_all_detector_samples_noleave_train[[month]][,counter]
  }
  noleave_train_detector_monthly_dists_list[[det]] <- bind_cols(dist_list)
}

### correlating the data across the months
...
time_correlated_leave_train_list <- list()
counter <- 0
for (det in all_detectors){
  counter <- counter+1
  ...
  copula_time_corr_samples <- data.frame(rCopula(copula = norm_cop_obj , n = nrow(as.data.frame(leave_train_detector_monthly_dists_list[[det]])))) #sample from copula object
  #correlating the training data across eight months
  time_correlated_leave_train_list[[det]] <- CorrelateData(DetectorData = as.data.frame(leave_train_detector_monthly_dists_list[[det]]), CopulaMatrix = as.data.frame(copula_time_corr_samples))
}

time_correlated_noleave_train_list <- list()
counter <- 0
for (det in all_detectors){
  counter <- counter+1
  #copula samples generating for eight months
  norm_cop_obj <- normalCopula(param = as.numeric(time_corr_mat[counter,]), dispstr = "un", dim = length(given_months)) # create gaussian copula oject
  copula_time_corr_samples <- data.frame(rCopula(copula = norm_cop_obj , n = nrow(as.data.frame(noleave_train_detector_monthly_dists_list[[det]])))) #sample from copula object
  #correlating the training data across eight months
  time_correlated_noleave_train_list[[det]] <- CorrelateData(DetectorData = as.data.frame(noleave_train_detector_monthly_dists_list[[det]]), CopulaMatrix = as.data.frame(copula_time_corr_samples))
}

### tranforming back to original format that is each matrix is monthly data for all detectors
time_correlated_leave_train_list_original <- list()
time_correlated_noleave_train_list_original <- list()
counter <- 0
for (month in given_months){
  counter <- counter+1
  time_correlated_leave_train_list_original[[month]] <- lapply(time_correlated_leave_train_list, function(df,month){df[,month]}, month = counter) %>% bind_cols
  time_correlated_noleave_train_list_original[[month]] <- lapply(time_correlated_noleave_train_list, function(df,month){df[,month]}, month = counter) %>% bind_cols
}


### end of code block for temporal correlation for training data ####
...
#loop for reading in the correlation files of leave and no leave group and making positive definite correlation matrices of them  
  ...
  avg_det_to_det_leave_train_corr <- Reduce("+", det_to_det_train_corr_leave_list) / length(det_to_det_train_corr_leave_list) # taking average of det-det correlation over months assuming constant trend
  avg_det_to_det_noleave_train_corr <- Reduce("+", det_to_det_train_corr_noleave_list) / length(det_to_det_train_corr_noleave_list) # taking average of det-det correlation over months assuming constant trend
  
  avg_time_correlated_leave_train_samples <- Reduce("+", time_correlated_leave_train_list_original) / length(time_correlated_leave_train_list_original) # taking average of the monthly data too
  avg_time_correlated_noleave_train_samples <- Reduce("+", time_correlated_noleave_train_list_original) / length(time_correlated_noleave_train_list_original) # taking average of the monthly data too
  
  #copula samples generating for leave group
  leave_norm_cop_obj <- normalCopula(param = P2p(as.data.frame(avg_det_to_det_leave_train_corr)), dispstr = "un", dim = nrow(as.data.frame(avg_det_to_det_leave_train_corr))) # create gaussian copula oject
  copula_train_leave_samples <- data.frame(rCopula(copula = leave_norm_cop_obj , n = nrow(as.data.frame(avg_time_correlated_leave_train_samples)))) #sample from copula object
  #copula samples generating for noleave group
  noleave_norm_cop_obj <- normalCopula(param = P2p(as.data.frame(avg_det_to_det_noleave_train_corr)), dispstr = "un", dim = nrow(as.data.frame(avg_det_to_det_noleave_train_corr))) # create gaussian copula oject
  copula_train_noleave_samples <- data.frame(rCopula(copula = noleave_norm_cop_obj , n = nrow(as.data.frame(avg_time_correlated_noleave_train_samples)))) #sample from copula object
  
  avg_time_detector_correlated_leave_train_samples <- CorrelateData(DetectorData = as.data.frame(avg_time_correlated_leave_train_samples), CopulaMatrix = as.data.frame(copula_train_leave_samples))
  avg_time_detector_correlated_noleave_train_samples <- CorrelateData(DetectorData = as.data.frame(avg_time_correlated_noleave_train_samples), CopulaMatrix = as.data.frame(copula_train_noleave_samples))
  ...
  for (month in given_months){
    ...
    correlated_train_leave_list[[month]] <- CorrelateDataByIndeces(UncorrelatedData = as.data.frame(time_correlated_leave_train_list_original[[month]]), CorrelatedData = as.data.frame(avg_time_detector_correlated_leave_train_samples))
    ...
    correlated_train_noleave_list[[month]] <- CorrelateDataByIndeces(UncorrelatedData = as.data.frame(time_correlated_noleave_train_list_original[[month]]), CorrelatedData = as.data.frame(avg_time_detector_correlated_noleave_train_samples))
  ...
  } else
  {
    ...
    ### loop to correlate the leave testing data ###
    ### transforming the data into detector distribution by month in each matrix. Each matrix contains the value of a particular detector for all months
    ...
    for (det in all_detectors){
      counter <- counter+1
      for (month in test_months){
        dist_list[[month]] <- list_of_all_detector_samples_leave_test[[month]][,counter]
      }
    ...
    for (det in all_detectors){
      counter <- counter+1
      for (month in test_months){
        dist_list[[month]] <- list_of_all_detector_samples_noleave_test[[month]][,counter]
      }
    ...
    #time_corr_mat_test <- suppressMessages(read_csv(str_c(getwd(), "/RCP17data/RCP17_time_spearman_correlations/Time_Correlations_Spearman.csv")))
    #time_corr_mat_test[,1] <- NULL #drop first column
    ...
    time_correlated_leave_test_list <- list()
    counter <- 0
    for (det in all_detectors){
      counter <- counter+1
      #copula samples generating for eight months
      norm_cop_obj <- normalCopula(param = as.numeric(time_corr_mat[counter,start_col:end_col]), dispstr = "un", dim = dim) # create gaussian copula oject
      copula_time_corr_samples <- data.frame(rCopula(copula = norm_cop_obj , n = nrow(as.data.frame(leave_test_detector_monthly_dists_list[[det]])))) #sample from copula object
      #correlating the testing data across three months
      time_correlated_leave_test_list[[det]] <- CorrelateData(DetectorData = as.data.frame(leave_test_detector_monthly_dists_list[[det]]), CopulaMatrix = as.data.frame(copula_time_corr_samples))
    }
    
    time_correlated_noleave_test_list <- list()
    counter <- 0
    for (det in all_detectors){
      counter <- counter+1
      #copula samples generating for eight months
      norm_cop_obj <- normalCopula(param = as.numeric(time_corr_mat[counter,start_col:end_col]), dispstr = "un", dim = dim) # create gaussian copula oject
      copula_time_corr_samples <- data.frame(rCopula(copula = norm_cop_obj , n = nrow(as.data.frame(noleave_test_detector_monthly_dists_list[[det]])))) #sample from copula object
      #correlating the training data across eight months
      time_correlated_noleave_test_list[[det]] <- CorrelateData(DetectorData = as.data.frame(noleave_test_detector_monthly_dists_list[[det]]), CopulaMatrix = as.data.frame(copula_time_corr_samples))
    }
    
    ### tranforming back to original format that is each matrix is monthly data for all detectors
    time_correlated_leave_test_list_original <- list()
    time_correlated_noleave_test_list_original <- list()
    counter <- 0
    for (month in test_months){
      counter <- counter+1
      time_correlated_leave_test_list_original[[month]] <- lapply(time_correlated_leave_test_list, function(df,month){df[,month]}, month = counter) %>% bind_cols
      time_correlated_noleave_test_list_original[[month]] <- lapply(time_correlated_noleave_test_list, function(df,month){df[,month]}, month = counter) %>% bind_cols
    }
    
    ### end of code block for temporal correlation for training data ####
    
    ### code block to induce det to det correlation into testing data
    
    avg_det_to_det_leave_test_corr <- Reduce("+", det_to_det_test_corr_leave_list) / length(det_to_det_test_corr_leave_list) # taking average of det-det correlation over months assuming constant trend
    avg_det_to_det_noleave_test_corr <- Reduce("+", det_to_det_test_corr_noleave_list) / length(det_to_det_test_corr_noleave_list) # taking average of det-det correlation over months assuming constant trend
    
    avg_time_correlated_leave_test_samples <- Reduce("+", time_correlated_leave_test_list_original) / length(time_correlated_leave_test_list_original) # taking average of the monthly data too
    avg_time_correlated_noleave_test_samples <- Reduce("+", time_correlated_noleave_test_list_original) / length(time_correlated_noleave_test_list_original) # taking average of the monthly data too
    
    #copula samples generating for leave group
    leave_norm_cop_obj <- normalCopula(param = P2p(as.data.frame(avg_det_to_det_leave_test_corr)), dispstr = "un", dim = nrow(as.data.frame(avg_det_to_det_leave_test_corr))) # create gaussian copula oject
    copula_test_leave_samples <- data.frame(rCopula(copula = leave_norm_cop_obj , n = nrow(as.data.frame(avg_time_correlated_leave_test_samples)))) #sample from copula object
    #copula samples generating for noleave group
    noleave_norm_cop_obj <- normalCopula(param = P2p(as.data.frame(avg_det_to_det_noleave_test_corr)), dispstr = "un", dim = nrow(as.data.frame(avg_det_to_det_noleave_test_corr))) # create gaussian copula oject
    copula_test_noleave_samples <- data.frame(rCopula(copula = noleave_norm_cop_obj , n = nrow(as.data.frame(avg_time_correlated_noleave_test_samples)))) #sample from copula object
    
    correlated_test_leave_list[[month]] <- CorrelateData(DetectorData = as.data.frame(avg_time_correlated_leave_test_samples), CopulaMatrix = as.data.frame(copula_test_leave_samples))
    ...
    correlated_test_noleave_list[[month]] <- CorrelateData(DetectorData = as.data.frame(time_correlated_noleave_test_list_original[[month]]), CopulaMatrix = as.data.frame(copula_test_noleave_samples))
    ...
    for (month in test_months){
      ...
      correlated_test_leave_list[[month]] <- CorrelateData(DetectorData = as.data.frame(time_correlated_leave_test_list_original[[month]]), CopulaMatrix = as.data.frame(copula_test_leave_samples))
      correlated_test_leave_list[[month]]["month"] <- rep(month, nrow(correlated_test_leave_list[[month]])) #adding month column
      correlated_test_leave_list[[month]]["label"] <- rep(1, nrow(correlated_test_leave_list[[month]]))
      correlated_test_noleave_list[[month]] <- CorrelateData(DetectorData = as.data.frame(time_correlated_noleave_test_list_original[[month]]), CopulaMatrix = as.data.frame(copula_test_noleave_samples))
      correlated_test_noleave_list[[month]]["month"] <- rep(month, nrow(correlated_test_noleave_list[[month]])) #adding month column
  ...
  write_csv(all_months_pop, str_c(getwd(), "/populations/all_months_2dcorr_pop_",org_num,".csv"))
